The BDC plugin design and tasks require several core APIs from the DdsMonitor engine to aggregate data and visualize entities. These APIs **are available** in the codebase, but as noted in the `MON-BATCH-28-REPORT.md`, some architectural changes were made to how plugins access them.
...
*   **Instance Lifecycle Tracking:** To aggregate descriptors, the `EntityStore` background engine must listen to DDS instance transitions (Alive, Disposed, NoWriters).
    *   *Availability:* **Yes.** The codebase implements `IInstanceStore.OnInstanceChanged`, which provides an `IObservable<InstanceTransitionEvent>` that the BDC plugin can subscribe to.
...
*   **Window Management & Pub/Sub:** The BDC UI panels must spawn new detail grids and broadcast events (like selecting an entity).
...
*   **Removal of Scoped Services from Context:** The original design stated `IMonitorContext` would contain `IWindowManager` and `ISampleStore`. Because `IWindowManager` is "Scoped" (one instance per browser tab) and plugins are "Singleton" (loaded once at startup), passing the window manager via context would cause a captive dependency issue. Therefore, the implemented `IMonitorContext` only exposes `IMenuRegistry` and `PluginPanelRegistry`.
*   **New Panel Registration Pattern:** The design expected plugins to register panels by calling `context.WindowManager.RegisterPanelType(...)`. To solve the lifecycle mismatch, plugins must now register their custom UI panels (like `BdcEntityGridPanel` and `EntityDetailPanel`) using `context.PanelRegistry.RegisterPanelType(...)`. The UI shell will safely resolve these registrations across all browser tabs.
...
1.  **Backend Services (`EntityStore`):** Instead of pulling `IInstanceStore` from the `IMonitorContext`, the BDC plugin should register its `EntityStore` as a background service inside its `ConfigureServices(IServiceCollection services)` method. The `EntityStore` will then automatically receive `IInstanceStore` via constructor injection.
2.  **UI Panels:** The custom BDC Blazor panels (`BdcEntityGridPanel`, etc.) will just use the standard `@inject ISampleStore` and `@inject IWindowManager` directives at the top of their Razor files to access the data and window APIs within the correct browser tab scope.
3.  **Initialization:** The plugin's `Initialize(IMonitorContext context)` method will strictly be used to register the custom BDC panels into the `PanelRegistry` and add any BDC-specific buttons to the `MenuRegistry`.
