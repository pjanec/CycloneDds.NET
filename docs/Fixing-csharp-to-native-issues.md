...
---- xxx
The issue with SequenceUnionAppendableTopic was that the **C# Serializer** was failing to emit a **DHEADER \(Delimiter Header\)** for the sequence&lt;SimpleUnionAppendable&gt; field, which is required because the containing struct SequenceUnionAppendableTopic is `@appendable`.

In **XCDR2**, when a struct is `@appendable`:

1. Primitive members are encoded normally.
2. **Constructed members** \(like `Sequence`, `Struct`, `Union`\) must be preceded by a **DHEADER** \(a 4-byte length indicator of the member's serialized size\).

The native `dds_stream_normalize` failed because it expected this DHEADER for the `unions` sequence but found the sequence length immediately. This caused a mismatch in the expected stream offset \(the "pos=57" error indicated it read past the expected end or hit a boundary\).

I have fixed this by updating SerializerEmitter.cs in the CycloneDDS.CodeGen tool to correctly emit DHEADERs for List&lt;T&gt; types when the containing struct is `@appendable` \(and encoding is XCDR2\).

### **Changes Applied**

**File:** SerializerEmitter.cs

1. **Patched EmitListWriter:**
Added logic to check if \(isAppendableStruct && isXcdr2\). If true, it writes a placeholder `UInt32(0)` before the list, records the position, writes the list, calculates the size, and patches the placeholder with the correct length \(DHEADER\).
2. **Patched EmitListSizer:**
Added logic to similarly align and reserve 4 bytes for the DHEADER in the size calculation process.

### **Verification**

After applying the fix and rebuilding the CodeGen tool, the validation error `dds_stream_normalize failed` disappeared, and the test proceeded to the Native Validation phase.

*\(Note: The test SequenceUnionAppendableTopic may still report a "Native expectation failed" with "double mismatch", which appears to be a separate issue with how the Native test runner validates specific Double values in Unions, potentially related to memory initialization or seed logic in the test fixture, but the blocking serialization stream validity issue is resolved.\)*

### **Remaining Tasks**

You may need to investigate atomic\_tests\_native.c further if the value mismatch persists, ensuring the native validator correctly accounts for the padding bytes shifted by the introduction of the Sequence DHEADER.
---
The SequenceUnionAppendableTopic failure \(and likely other complex XCDR2 deserialization issues\) has been fixed. The root cause was twofold:

1. **Missing Sequence Header Consumption**: In XCDR2, appendable structs/unions introduce a "Delimiter Header" \(DHEADER\) before the sequence length. The deserializer was failing to consume this 4-byte header, causing it to interpret the header value \(16\) as the sequence length, leading to an out-of-bounds read and subsequent crash.
2. **Incorrect Alignment in XCDR2**: The deserializer was enforcement strict 4-byte or native alignment logic suitable for XCDR1. XCDR2 generally packs data with 1-byte alignment \(except for top-level headers\). The generated code was calling reader.Align\(4\) or reader.Align\(N\) inside XCDR2 streams, which misaligned the read pointer.

I applied the following fixes to DeserializerEmitter.cs:

- Updated EmitSequenceReader and EmitListReader to conditionally invoke alignment: reader.Align\(reader.Encoding == CdrEncoding.Xcdr2 ? 1 : alignment\). This ensures XCDR2 streams remain packed while XCDR1 streams respect alignment.
- Ensured the "Sequence Header" \(DHeader\) skip logic is guarded by both IsAppendable and the XCDR2 encoding check.

**Verification Results:**
The SequenceUnionAppendableTopic test now passes successfully in both directions \(C-&gt;C# and C#-&gt;C\).
---