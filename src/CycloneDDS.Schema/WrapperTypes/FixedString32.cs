        ...
        try
        {
            ...
            fixed (char* charPtr = value)
            {
                fixed (byte* destPtr = result._buffer)
                {
                    ...
                    // No need to explicitly NUL-terminate if we zero-initialized and byteCount < Capacity.
                    // If byteCount == Capacity, it fills the buffer, no NUL needed if we treat full buffer as string.
                    // But Length property scans until NUL or Capacity.
                }
            }
        }
        catch (EncoderFallbackException)
