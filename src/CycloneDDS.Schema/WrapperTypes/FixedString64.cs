...
/// <summary>
/// A fixed-size string with a maximum capacity of 64 bytes (UTF-8 encoded, NUL-terminated).
/// </summary>
...
public unsafe struct FixedString64
{
    ...
    /// <summary>
    /// Gets the maximum capacity in bytes (64).
    /// </summary>
    ...
    /// <summary>
    /// Creates a new FixedString64 from a string. Throws if too long or invalid UTF-8.
    /// </summary>
    ...
    public FixedString64(string value)
    {
        if (!TryFrom(value, out this))
        {
            ...
            catch (EncoderFallbackException)
            {
             ...
             if (count > Capacity)
                throw new ArgumentException($"String provided is too long ({count} bytes) for FixedString64.", nameof(value));
             else
    ...
    /// <summary>
    /// Tries to create a FixedString64 from a string.
    /// </summary>
    /// <param name="value">The source string.</param>
    /// <param name="result">The resulting FixedString64.</param>
    /// <returns>True if successful; otherwise false (e.g. if too long or invalid UTF-8).</returns>
    public static bool TryFrom(string value, out FixedString64 result)
    {
        result = default;
