...
namespace CycloneDDS.CodeGen
{
    public class CodeGenerator
    {
        ...
        public void Generate(string sourceDir, string outputDir, IEnumerable<string>? referencePaths = null)
        {
            ...
            foreach (var topic in types)
            {
                if (topic.IsTopic || topic.IsStruct || topic.IsUnion)
                {
                    ...
                    sb.AppendLine("#pragma warning disable CS0162, CS0219, CS8600, CS8601, CS8602, CS8603, CS8604, CS8605, CS8618, CS8625, CS8669");
                    ...
                    // Single combined partial class with all generated members
                    string typeKind = topic.IsClass ? "class" : "struct";
                    sb.AppendLine($"    // <auto-generated />");
                    sb.AppendLine($"    public partial {typeKind} {topic.Name}");
                    sb.AppendLine("    {");

                    // Serializer body (marshaller, sizer, unmarshal, etc.)
                    sb.Append(_serializerEmitter.EmitSerializerClassBody(topic, registry));

                    // Descriptor body (ops, keys, flagset, size, align)
                    if (descriptors.TryGetValue(topic.FullName, out var descriptor))
                    {
                        sb.Append(EmitDescriptorClassBody(topic, descriptor));
                    }
                    ...
                    // Ghost struct(s) – outside partial class, inside namespace
                    sb.AppendLine(_serializerEmitter.EmitGhostStructContent(topic));

                    // View ref struct – separate type inside namespace
                    sb.AppendLine(_viewEmitter.EmitViewStructInsideNamespace(topic, registry));
                    sb.AppendLine();

                    // Extensions static class – separate type inside namespace
                    sb.AppendLine(_viewExtensionsEmitter.EmitExtensionsInsideNamespace(topic, registry));
        ...
        /// <summary>
        /// Returns only the body members for the descriptor partial class (no namespace or class wrapper).
        /// Used by the combined code generator to emit a single partial class definition.
        /// </summary>
        private string EmitDescriptorClassBody(TypeInfo topic, JsonTopicDescriptor descriptor)
        {
            var sb = new System.Text.StringBuilder();
