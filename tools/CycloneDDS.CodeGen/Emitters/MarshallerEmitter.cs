...
public class MarshallerEmitter
{
    ...
    public string GenerateMarshaller(TypeDeclarationSyntax type, string namespaceName)
    {
        ...
        var arrayFields = fields.Where(f => f.Declaration.Type.ToString().EndsWith("[]")).ToList();
        ...
        // Class declaration with IDisposable if has arrays
        if (hasArrays)
        {
            EmitLine($"public class {marshallerTypeName} : IMarshaller<{typeName}, {nativeTypeName}>, IDisposable");
        }
        else
        {
        ...
        // Private fields to track allocated pointers
        if (hasArrays)
        {
        ...
        {
            EmitMarshalField(field, hasArrays);
        }
        ...
        if (hasArrays)
        {
            ...
            foreach (var field in arrayFields)
            {
                ...
                EmitLine("        {");
                EmitLine($"            System.Runtime.InteropServices.Marshal.FreeHGlobal(_allocated{fieldName}_Ptr);");
    ...
    private void EmitMarshalField(FieldDeclarationSyntax field, bool trackAllocations)
    {
        ...
        if (fieldType.EndsWith("[]"))
        {
            ...
            if (IsPrimitive(elementType))
            {
                ...
                if (trackAllocations)
                {
                    EmitLine($"            _allocated{fieldName}_Ptr = native.{fieldName}_Ptr; // Track for disposal");
                }
